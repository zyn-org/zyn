...
use entangle_server::version::VERSION;

const ENV_WORKER_THREADS: &str = "ENTANGLE_WORKER_THREADS";

/// Command line arguments
#[derive(Parser, Debug)]
#[command(name = "entangle-server")]
#[command(version = VERSION)]
#[command(about = "Entangle server", long_about = None)]
struct Cli {
...
fn main() {
  entangle_server::setup_panic_hook();
  ...
  runtime.block_on(async {
    match entangle_server::run(cli.config, worker_threads).await {
      Ok(_) => {},
      Err(e) => {
        eprintln!("error: {}", e);
        std::process::exit(1);
