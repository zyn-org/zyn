...
use zyn_protocol::{IdentifyParameters, Message};
use zyn_test_util::{C2sSuite, assert_message, default_c2s_config};
...
async fn test_c2s_username_in_use() -> anyhow::Result<()> {
  ...
  assert_message!(
    tls_socket.read_message().await?,
    Message::Error,
    ErrorParameters { id: None, reason: zyn_protocol::ErrorReason::UsernameInUse.into(), detail: None }
  );
...
async fn test_c2s_join_non_existing_channel() -> anyhow::Result<()> {
  ...
  assert_message!(
    suite.read_message(TEST_USER_1).await?,
    Message::Error,
    ErrorParameters { id: Some(2), reason: zyn_protocol::ErrorReason::ChannelNotFound.into(), detail: None }
  );
...
async fn test_c2s_join_full_channel() -> anyhow::Result<()> {
  ...
  assert_message!(
    suite.read_message(TEST_USER_2).await?,
    Message::Error,
    ErrorParameters { id: Some(2), reason: zyn_protocol::ErrorReason::ChannelIsFull.into(), detail: None }
  );
...
async fn test_c2s_join_more_than_once() -> anyhow::Result<()> {
  ...
  assert_message!(
    suite.read_message(TEST_USER_1).await?,
    Message::Error,
    ErrorParameters { id: Some(2), reason: zyn_protocol::ErrorReason::UserInChannel.into(), detail: None }
  );
...
async fn test_c2s_non_member_leave() -> anyhow::Result<()> {
  ...
  assert_message!(
    suite.read_message(TEST_USER_2).await?,
    Message::Error,
    ErrorParameters { id: Some(1), reason: zyn_protocol::ErrorReason::UserNotInChannel.into(), detail: None }
  );
...
async fn test_c2s_unauthorized_channel_configuration() -> anyhow::Result<()> {
  ...
  assert_message!(
    suite.read_message(TEST_USER_2).await?,
    Message::Error,
    ErrorParameters { id: Some(1234), reason: zyn_protocol::ErrorReason::Forbidden.into(), detail: None }
  );
...
async fn test_c2s_channel_acl_join_deny() -> anyhow::Result<()> {
  ...
  assert_message!(
    suite.read_message(TEST_USER_2).await?,
    Message::Error,
    ErrorParameters { id: Some(2), reason: zyn_protocol::ErrorReason::NotAllowed.into(), detail: None }
  );
...
async fn test_c2s_channel_acl_publish_deny() -> anyhow::Result<()> {
  ...
  assert_message!(
    suite.read_message(TEST_USER_2).await?,
    Message::Error,
    ErrorParameters { id: Some(1234), reason: zyn_protocol::ErrorReason::NotAllowed.into(), detail: None }
  );
